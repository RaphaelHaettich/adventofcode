...
const getLongestSleepMinutes = (duplicateTimesAndLength) => {
    ...
    duplicateTimesAndLength.forEach((elem) => {
        if (elem.sleepTime > highestTimeObj.sleepTime) {
            highestTimeObj = elem;
        }
    })

    let highestSleepMinute = {time: 0};

    for (var [key, value] of highestTimeObj.timeMap) {
        if (value > highestSleepMinute.time) {
            highestSleepMinute = {time: value, minute: key};
        }
    }

    return { guardId: highestTimeObj.key, minuteMaxSleep: highestSleepMinute.minute };
